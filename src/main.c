 ...
 * v0.5 - auto-aceleracao + diagnostico de input
 */
#include <genesis.h>
...
int main(void)
{
    ...
    while (1)
    {
        ...
        /* AUTO-ACELERACAO: anda sozinho ate 6000;
           segurando CIMA vai ate 12000; BAIXO freia */
        if (joy & BUTTON_UP)        speed += 300;
        else if (joy & BUTTON_DOWN) speed -= 500;
        else if (speed < 6000)      speed += 150;
        else                        speed -= 100;
        ...
        /* HUD: velocidade + tempo + setas de input */
        sprintf(txt, "%d", (int)(speed / 100));
        VDP_drawText("VEL", 2, 0);
        VDP_drawText(txt, 6, 0);
        sprintf(txt, "%d", (int)(frame / 60));
        VDP_drawText("T", 12, 0);
        VDP_drawText(txt, 14, 0);
        VDP_drawText((joy & BUTTON_LEFT)  ? "<" : " ", 20, 0);
        VDP_drawText((joy & BUTTON_RIGHT) ? ">" : " ", 21, 0);

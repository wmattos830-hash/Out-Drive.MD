 ...
 * v0.4 - pista pseudo-3D + carro + controles
 */
#include <genesis.h>
...
{
    static const u8 off[8] = {0,1,2,3,4,5,6,7};
    VDP_clearTileMapRect(BG_A, 0, 22, 40, 6);
    for (u16 i = 0; i < 8; i++)
    {
        u16 tx = carX + (i & 3);
        u16 ty = 23 + (i >> 2);
        u16 tile = TA(PAL0, 160 + off[i]);
        VDP_setTileMapXY(BG_A, tile, tx, ty);
    }
}
...
int main(void)
{
    ...
    while (1)
    {
        ...
        if (joy & BUTTON_UP)   speed += 300; else speed -= 100;
